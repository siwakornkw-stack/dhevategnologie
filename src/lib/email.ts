...
const FROM = process.env.EMAIL_FROM ?? 'noreply@dhevategnologie.com';
...
export async function sendVerificationEmail(to: string, token: string) {
  ...
  await sendEmail({
    from: FROM,
    to,
    subject: 'Verify your email / ยืนยันอีเมลของคุณ - Dhevategnologie',
    html: emailTemplate({
      titleEn: 'Verify Your Email',
      titleTh: 'ยืนยันอีเมล',
      emoji: '📧',
      bodyEn: `
        <p>Thank you for registering with Dhevategnologie!</p>
      ...
      `,
      bodyTh: `
        <p>ขอบคุณที่สมัครสมาชิก Dhevategnologie!</p>
...
export async function sendPasswordResetEmail(to: string, token: string) {
  ...
  await sendEmail({
    from: FROM,
    to,
    subject: 'Reset your password / รีเซ็ตรหัสผ่าน - Dhevategnologie',
    html: emailTemplate({
